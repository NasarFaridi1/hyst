...
class OrderController extends Controller
{
    ...
    public function checkout()
    {
        ...
        if (($request->order_type === 'takeaway' || $request->order_type === 'delivery') && $originalTotal > 250) {
            return response()->json([
                'status'  => false,
                'message' => ucfirst($request->order_type) . ' orders cannot exceed £250. Please adjust your order amount.'
            ], 422);
        }
        ...
        if ($request->order_type === 'dine_in') {
            ...
            $subAfterOffer = max(0, $originalTotal - $discount);
            $rawDelivery = (float) $request->input('delivery_charge', $deliveryCharge);
            if ($subAfterOffer < 20) {
                if ($rawDelivery > 3.99) {
                    $offset = 1.99;
    ...
    /**
     * Customer Update Delivery Notes (dropoff_notes) for active Uber Direct delivery via API
     */
    public function updateDropoffNotes(Request $request, $id)
    {
        ...
        if (!empty($order->uber_delivery_id)) {
            try {
                ...
                Log::warning("API Customer Uber update delivery note warning for Order #{$id}: " . $e->getMessage());
            }
        }
