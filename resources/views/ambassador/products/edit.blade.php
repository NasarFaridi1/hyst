...
<style>
    ...
    .field-textarea { width:100%; border:1.5px solid #EBE5DE; padding:12px 14px 12px 40px; border-radius:12px; font-family:'Poppins',sans-serif; font-size:14px; color:#0D0D0D; background:#FDFAF7; outline:none; box-sizing:border-box; resize:vertical; min-height:110px; transition:border-color 0.18s,background 0.18s; }
    ...
    .current-img-wrap { display:flex; align-items:center; gap:12px; padding:10px 14px; background:#F5F0E8; border-radius:10px; margin-bottom:10px; }
    .current-img { width:60px; height:60px; border-radius:10px; object-fit:cover; border:1.5px solid #EBE5DE; }
    .current-img-label { font-size:12px; color:#888; font-weight:500; }
    .current-img-label span { display:block; font-size:11px; color:#BBB; }
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-sub">Update details for <strong style="color:#C25A2A;">{{ $product->name }}</strong></p>
        </div>
    ...
    <div class="hyst-card">
        ...
        <div class="card-head">
            ...
            <div>
                <p class="card-head-title">Edit Product Details</p>
                <p class="card-head-sub">Changes will update immediately after saving</p>
            </div>
        </div>

        <form method="POST" enctype="multipart/form-data" action="{{ route('ambassador.products.update',[$restaurant->id,$product->id]) }}">
            @csrf
            @method('PUT')
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <div class="field-wrap">
                            ...
                            <input type="text" name="name" value="{{ old('name',$product->name) }}" placeholder="Product name" class="field-input">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="field-wrap">
                            ...
                            <input type="number" step="0.01" name="price" value="{{ old('price',$product->price) }}" placeholder="0.00" class="field-input">
                        </div>
                    </div>
                    ...
                    <div class="form-group full">
                        ...
                        <div class="field-wrap">
                            ...
                            <textarea name="description" placeholder="Describe the product…" class="field-textarea">{{ old('description',$product->description) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-group full">
                        ...
                        @if($product->image)
                            <div class="current-img-wrap">
                                <img src="{{ asset('storage/'.$product->image) }}" class="current-img" id="img-preview-edit">
                        ...
                        <label class="file-zone">
                            <input type="file" name="image" accept="image/*" onchange="previewEdit(this)">
                            <div class="file-zone-icon">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                            </div>
                            <div class="file-zone-text">
                                <span>Choose new file</span> or drag &amp; drop
...
function previewEdit(input) {
    ...
    if (!preview) {
        ...
        preview.id = 'img-preview-edit';
        preview.style.cssText = 'width:60px;height:60px;border-radius:10px;object-fit:cover;border:1.5px solid #EBE5DE;';
        input.closest('.form-group').insertBefore(preview, input.closest('.file-zone'));
    }
