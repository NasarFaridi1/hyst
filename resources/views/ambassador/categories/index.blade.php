...
<div class="max-w-7xl mx-auto">
    ...
    <div class="page-header">
        <div>
            <h1 class="page-title">{{ $restaurant->name }} — Categories</h1>
            <p class="page-sub">Manage food categories for this restaurant</p>
        </div>
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
            ...
            <a href="{{ route('ambassador.categories.create',$restaurant->id) }}" class="btn-add">
    ...
    <div class="hyst-card">
        ...
        <form method="GET" class="search-bar">
            <div class="search-wrap">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search category by name..." class="search-input">
            </div>
            ...
            @if(request('search'))
                <a href="{{ route('ambassador.categories.index', $restaurant->id) }}" class="btn-reset">
        ...
        <table class="hyst-table">
            ...
            @forelse($categories as $i => $category)
                <tr>
                    <td style="color:#BBB; font-size:12px; font-weight:600;">{{ $categories->firstItem() + $i }}</td>
                    <td>
                        @if($category->image)
                            <img src="{{ asset('storage/'.$category->image) }}" class="cat-img">
                        @else
                    ...
                    <td style="font-weight:600; color:#0D0D0D;">{{ $category->name }}</td>
                    <td>
                    ...
                    <td>
                        <div class="action-group">
                            <a href="{{ route('ambassador.categories.edit',[$restaurant->id,$category->id]) }}" class="btn-action btn-edit">
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                Edit
                            </a>
                            <form action="{{ route('ambassador.categories.destroy',[$restaurant->id,$category->id]) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Delete this category?')" class="btn-action btn-del">
                    ...
                    <td colspan="6">
                        <div class="empty-state">
                            <div class="empty-state-icon">
                                <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
                            </div>
                            <p>No categories found{{ request('search') ? ' for "'.request('search').'"' : '' }}</p>
                        </div>
                    </td>
                </tr>
            @endforelse
