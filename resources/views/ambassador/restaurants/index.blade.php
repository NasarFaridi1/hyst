...
<style>
    ...
    .restaurant-name { font-weight: 700; color: #0D0D0D; font-size: 13.5px; }
    .restaurant-email { font-size: 12px; color: #999; margin-top: 2px; }
    ...
    .badge-pending { background: #FEF0EE; color: #D94F3D; }
    ...
    .btn-cat    { background: #EEF2FF; color: #4F46E5; }
    .btn-prod   { background: #ECFDF5; color: #059669; }
...
<div class="max-w-7xl mx-auto">
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-sub">Manage and monitor your restaurant listings</p>
        </div>
        <a href="{{ route('ambassador.restaurants.create') }}" class="btn-add">
    ...
    <div class="hyst-card">
        ...
        <form method="GET" class="search-bar">
            <div class="search-wrap">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search restaurant by name..." class="search-input">
            </div>
            ...
            @if(request('search'))
                <a href="{{ route('ambassador.restaurants.index') }}" class="btn-reset">
        ...
        <table class="hyst-table">
            ...
            @forelse($restaurants as $i => $restaurant)
                <tr>
                    <td style="color:#BBB; font-size:12px; font-weight:600;">{{ $restaurants->firstItem() + $i }}</td>
                    <td>
                        @if($restaurant->image)
                            <img src="{{ asset('storage/'.$restaurant->image) }}" class="restaurant-img">
                        @else
                            <div class="img-placeholder">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 2h18l-2 7H5L3 2z"/><path d="M5 9v13h14V9"/></svg>
                            </div>
                        @endif
                    </td>
                    <td>
                    ...
                    <td>
                        <div class="action-group">
                            <a href="{{ route('ambassador.restaurants.edit',$restaurant->id) }}" class="btn-action btn-edit">
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                Edit
                            </a>
                            <a href="{{ route('ambassador.categories.index',$restaurant->id) }}" class="btn-action btn-cat">
                            ...
                            <a href="{{ route('ambassador.products.index',$restaurant->id) }}" class="btn-action btn-prod">
                            ...
                            <form action="{{ route('ambassador.restaurants.destroy',$restaurant->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Delete this restaurant?')" class="btn-action btn-del">
                    ...
                    <td colspan="7">
                        <div class="empty-state">
                            <div class="empty-state-icon">
                                <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 2h18l-2 7H5L3 2z"/><path d="M5 9v13h14V9"/></svg>
                            </div>
                            <p>No restaurants found{{ request('search') ? ' for "'.request('search').'"' : '' }}</p>
                        </div>
                    </td>
                </tr>
            @endforelse
