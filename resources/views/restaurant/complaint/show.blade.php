...
<div class="od-wrap">
  ...
  <div class="grid grid-cols-1 md:grid-cols-4 xl:grid-cols-4 gap-8 mb-8">
    ...
    <div class="od-card">
      ...
      @php
        $complaintTrackUrl = $order->uber_tracking_url ?? $order->tracking_url;
      @endphp
      @if($complaintTrackUrl)
        <a href="{{ $complaintTrackUrl }}" target="_blank" rel="noopener noreferrer" onclick="window.open(this.href, '_blank'); return false;" class="track-link">
    ...
    <div class="od-card">
        ...
        <div class="card-eyebrow">
            Complaint #{{ $complaint->id }} — Status
        </div>

        <div style="margin-bottom:14px;">
            <span class="badge badge-{{ $complaint->status == 'resolved' ? 'completed' : ($complaint->status == 'rejected' ? 'cancelled' : ($complaint->status == 'open' ? 'pending' : 'accepted')) }}">
                {{ ucfirst(str_replace('_',' ', $complaint->status)) }}
            </span>
        </div>

        <form action="{{ route('restaurant.complaint.status', $complaint->id) }}" method="POST">
            @csrf
...
@endforeach
	  ...
	  @if($order->status === 'completed' && $customerEvidence)
...
@endforeach
                    ...
                    <form method="POST"
                          action="{{ route('restaurant.complaint.reply',$complaint->id) }}">
